...
"""
Problem: Verilen my_str içindeki her bir elemanı önce ayrıştırılacak ve sayılabilir olacak. Bu sayede my_str
içerisinde yer alan metinin sözcük frekansını hesaplamamız gerekiyor.
Solution: Öncelikle split(), string metoduyla metini öğelere ayırıyoruz. Ardından boş bir dictionary yaratıp,
yarattığımız 'myDict' içerisine for döngüsünü kullanarak bir yandan metin içerisindeki sözcükleri sayarken, 
diğer yandan yarattığımız boş dictionary içerisine key ve value olarak önce sözcük ardından count metoduyla 
sözcüğün sayısını yazdırıyoruz. 
